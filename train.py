...
weights = {
    'wc1': tf.Variable(tf.truncated_normal([5, 5, 1, 12], stddev=0.05), name='wc1'),
    'wc2': tf.Variable(tf.truncated_normal([5, 5, 12, 12], stddev=0.05), name='wc2'),
    'wc3': tf.Variable(tf.truncated_normal([5, 5, 12, 12], stddev=0.05), name='wc3'),
    'wc4': tf.Variable(tf.truncated_normal([5, 5, 12, 24], stddev=0.05), name='wc4'),
    'wc5': tf.Variable(tf.truncated_normal([5, 5, 24, 24], stddev=0.05), name='wc5'),
    'wc6': tf.Variable(tf.truncated_normal([5, 5, 24, 24], stddev=0.05), name='wc6'),
    'wfc': tf.Variable(tf.truncated_normal([24, 128]), name='wfc')
}

biases = {
    'bc1': tf.Variable(tf.truncated_normal([12]), name='bc1'),
    'bc2': tf.Variable(tf.truncated_normal([12]), name='bc2'),
    'bc3': tf.Variable(tf.truncated_normal([12]), name='bc3'),
    'bc4': tf.Variable(tf.truncated_normal([24]), name='bc4'),
    'bc5': tf.Variable(tf.truncated_normal([24]), name='bc5'),
...
def conv_layer(x, w_key, b_key, k=1):
...
def predict(x):
    ...
    conv1 = conv_layer(x, 'wc1', 'bc1', 4)
    conv2 = conv_layer(conv1, 'wc2', 'bc2')
    conv3 = conv_layer(conv2, 'wc3', 'bc3')
    pool1 = pool_layer(conv3)
    conv4 = conv_layer(pool1, 'wc4', 'bc4', 4)
    conv5 = conv_layer(conv4, 'wc5', 'bc5')
    conv6 = conv_layer(conv5, 'wc6', 'bc6')
    pool2 = pool_layer(conv6)

    return fully_connected_layer(pool2, 24, 'wfc', 'bfc')
